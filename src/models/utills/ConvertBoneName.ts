...
export function ConvertBoneName(vrmBoneName: VRMSchema.HumanoidBoneName): string {
    let result: string = vrmBoneName

    if (vrmBoneName.includes('Thumb')) result = InsertAfterKey(result, 'Thumb', '_')
...
function InsertAfterKey(baseString: string, key: string, insertString: string): string {
